...
class MultiLossLayer(tf.keras.layers.Layer):
    ...
    def call(self,targets,logits_list):
        losses=[]
        for logits in logits_list:
            loss = transformer_loss(logits, 
                                    targets, 
                                    self.label_smoothing,
                                    self.vocab_size)
            losses.append(loss)
        losss = tf.reduce_mean(tf.add_n(losses))
        self.add_loss(losss)
        
            
def computeMultiLoss(vocab_len,targets,logits_list,smoothing=0.1):
    def transformer_loss(logits, labels, smoothing, vocab_size):
        """Calculates total loss containing cross entropy with padding ignored.
    ...
    if len(logits_list) >1:
        for logits in logits_list:
            xentropy, weights  = transformer_loss(logits, 
                                        targets, 
                                        smoothing,
                                        vocab_len)
            losses.append(xentropy)
        losss = tf.reduce_sum(tf.add_n(losses))/tf.reduce_sum(weights)
        return losss
    else:
        xentropy, weights  = transformer_loss(logits_list[-1], 
                                        targets, 
                                        smoothing,
                                        vocab_len)
        return tf.reduce_sum(xentropy) / tf.reduce_sum(weights)
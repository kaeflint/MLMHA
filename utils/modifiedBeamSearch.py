...
class SequenceBeamSearch(object):
  ...
  def __init__(self, symbols_to_logits_fn, vocab_size, batch_size,
               ...
               max_decode_length,
               eos_id,
               average_target_length=200,
              normalization_funct=1,
              ):
    ...
    #The length normalization functions
    self.normalization_funct = normalization_funct
    #print('Norm function ',self.normalization_funct )
    self.average_target_length = average_target_length
  ...
  def _continue_search(self, state):
    ...
    #_length_normalization(alpha,length,average_target_length,funct_type)
    max_length_norm = _length_normalization(self.alpha, self.max_decode_length,self.average_target_length,self.normalization_funct)
  ...
  def _grow_alive_seq(self, state):
    ...
    #print('log_probs ',log_probs.shape,' Bs: ',self.beam_size,' vs: ',self.vocab_size)
  ...
  def _get_new_finished_state(self, state, new_seq, new_log_probs):
    ...
    length_norm = _length_normalization(self.alpha, i + 1,self.average_target_length,self.normalization_funct) #_length_normalization(self.alpha, i + 1)
...
def f1_length_normalization(alpha, length):
    """Return length normalization factor."""
    f_1 = tf.pow(((5. + tf.cast(length, tf.float32)) / 6.), alpha)
    return f_1

def f2_length_normalization(alpha,length):
    f_2= tf.pow((1. +tf.cast(length, tf.float32)),alpha)
    return tf.cast(f_2,tf.float32)

def min_hinge_normalization(length,average_target_length):
    mh= tf.cast(tf.minimum(length,average_target_length),tf.float32)
    return mh

def _length_normalization(alpha,length,average_target_length,funct_type):
    if funct_type==1:
        return f1_length_normalization(alpha, length)
    elif funct_type==2:
        return f2_length_normalization(alpha, length)
    elif funct_type==0:
        return min_hinge_normalization( length,average_target_length)

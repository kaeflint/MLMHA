...
class MetricLayer(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.metric_mean_fns = [
        (tf.keras.metrics.Mean(self.metric_name+"_accuracy"), padded_accuracy),
        (tf.keras.metrics.Mean(self.metric_name+"_accuracy_top5"), padded_accuracy_top5),
        (tf.keras.metrics.Mean(self.metric_name+"_accuracy_per_sequence"),
         padded_sequence_accuracy),
        (tf.keras.metrics.Mean(self.metric_name+"_neg_log_perplexity"), neg_log_perplexity),
    ]
...
def _pad_tensors_to_same_length_multi(x, y):
    """Pad x and y so that the results have the same length (second dimension)."""
    with tf.name_scope("pad_to_same_length"):
        x_last =x[-1]
        x_length = tf.shape(x_last)[1]
        y_length = tf.shape(y)[1]
        max_length = tf.maximum(x_length, y_length)
        x = [tf.pad(xl, [[0, 0], [0, max_length - x_length], [0, 0]]) for xl in x]
        y = tf.pad(y, [[0, 0], [0, max_length - y_length]])
        return x, y
def padded_cross_entropy_loss_multi(multi_logits, labels, smoothing, vocab_size):
    with tf.name_scope("loss"):
        multi_logits, labels = _pad_tensors_to_same_length_multi(multi_logits, labels)
        # Calculate smoothing cross entropy
        with tf.name_scope("smoothing_cross_entropy"):
            confidence = 1.0 - smoothing
            low_confidence = (1.0 - confidence) / tf.cast(vocab_size - 1, tf.float32)
            soft_targets = tf.one_hot(tf.cast(labels, tf.int32),
                              ...
                              off_value=low_confidence)
            xentropy = [tf.nn.softmax_cross_entropy_with_logits(
                logits=logits, labels=soft_targets) for logits in multi_logits]
            xentropy = tf.add_n(xentropy)
            normalizing_constant = -(confidence * tf.math.log(confidence) +
                                      tf.cast(vocab_size - 1, tf.float32) * low_confidence *
                                      tf.math.log(low_confidence + 1e-20))
        ...
        wxentropy = xentropy * weights
        #print(tf.reduce_sum((xentropy+normalizing_constant)* weights)/ tf.reduce_sum(weights))
        return wxentropy, weights
def transformer_loss_multi(multi_logits, labels, smoothing, vocab_size):
    """Calculates total loss containing cross entropy with padding ignored.
      Args:
        logits: Tensor of size [batch_size, length_logits, vocab_size]
        labels: Tensor of size [batch_size, length_labels]
      ...
      Returns:
        A scalar float tensor for loss.
    """
    print('Computing the transformer for the Self-Multi-task learning')
    xentropy, weights = padded_cross_entropy_loss_multi(multi_logits, labels, smoothing,vocab_size)
    return tf.reduce_sum(xentropy) / tf.reduce_sum(weights)
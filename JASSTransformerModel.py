...
class EncoderStack(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        self.aggregateFunction = None
        self.with_global = params.get('with_global',False)
        if params.get('with_global',False):
           self.aggregateFunction = params.get('agg_fn','lc')
           print('MLMHA with Global Representation')
    ...
    def build(self, input_shape):
        ...
        #Create the final normalization layer for each vector within the diversity range
...
class DecoderStack(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        self.layerWeight=params.get('layerweight',False)
        ...
        if params['top_n'] is None:
            self.last_n = params["num_hidden_layers"]
        cc= self.last_n if not self.with_global else self.last_n+1
        ...
        for _ in range(params["num_hidden_layers"]):
            ...
            enc_dec_attention_layer = JASSAttentionLayer(
              params["hidden_size"], 
                params["num_heads"],
              params["attention_dropout"],
                nb_links=self.last_n if not self.with_global else self.last_n+1,
...
class JASSTransformerLayer(tf.keras.layers.Layer):
    def __init__(self,params,
                 name=None):
        super(JASSTransformerLayer, self).__init__(name=name)
        self.params = params
    ...
    def build(self, input_shape):
        ...
        params = self.params
        self.built=True
        super(JASSTransformerLayer, self).build(input_shape)
...
def buildBasicJASSModelLayer(params,
    ...
    internal_model = JASSTransformerLayer(params=params, 
                                               name="transformer")
